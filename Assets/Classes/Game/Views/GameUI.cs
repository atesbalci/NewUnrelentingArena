...
namespace Game.Views
{
    ...
    {
        [Header("Scores")] public RectTransform Scores;
        ...
        public void Init(GameNetworkSynchronizer sync)
        {
            ...
            MessageManager.ReceiveEvent<GameStateChangedEvent>().Subscribe(ev => { RefreshUI(); });
            MessageManager.ReceiveEvent<PlayerListChangedEvent>().Subscribe(ev => { ObserveScores(ev.Players); });
            RefreshUI();
        }
        ...
        public void ObserveScores(System.Collections.Generic.Dictionary<uint, PlayerData> players)
        {
            ...
            {
                pl.Value.Points.AsObservable().Subscribe(val => { refresh(); });
            }
            refresh();
        }
    }
